...
\begin{itemize}
    \item \texttt{networkx}: Visualização de grafos
    \item \texttt{matplotlib}: Geração de gráficos
    \item \texttt{numpy} e \texttt{scipy}: Regressão linear e cálculos numéricos
    \item Implementação personalizada da estrutura de grafo (\texttt{graph.py})
\end{itemize}

Os detalhes da implementação — estrutura de dados, inserção de arestas, cálculo de métricas e ajuste de lei de potência — são documentados separadamente no documento de implementação técnica (\texttt{implementacao.tex}).
...
\begin{itemize}
    ...
    \item Distribuição de graus e ajuste de lei de potência
\end{itemize}
...
O valor do expoente $\gamma = 2,0416$ situa-se no intervalo $2 \leq \gamma < 3$, o que caracteriza uma \textbf{rede scale-free típica com média finita e variância infinita}, conforme indicado na Figura \ref{fig:power_law}. Este resultado tem importantes implicações:
...
A Figura \ref{fig:graph_partial} apresenta uma visualização parcial da rede, exibindo os 10 primeiros vértices e todos os seus vizinhos diretos.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{graph_10.png}
    \caption{Visualização parcial da rede de colaboração: 10 vértices iniciais e seus respectivos vizinhos diretos}
...
\begin{itemize}
    ...
    \item Presença de vértices com muitos vizinhos (hubs locais), como os nós 17038, 3466 e 1854
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Visualização completa da rede de colaboração científica ca-GrQc com todos os 5.242 vértices.}
    \label{fig:graph_full}
\end{figure}

Devido à alta densidade de nós e arestas, a visualização completa apresenta-se como uma massa compacta, o que dificulta a identificação de estruturas individuais. No entanto, é possível observar:

\begin{itemize}
    \item \textbf{Núcleo central denso}: região escura no centro, onde se concentram os pesquisadores com maior número de colaborações
    \item \textbf{Borda mais esparsa}: nós periféricos com menos conexões formam uma camada exterior mais clara
\end{itemize}

A identificação detalhada de comunidades e hubs individuais requereria técnicas de visualização específicas para redes de grande escala, como detecção de comunidades ou visualização de subgrafos.
